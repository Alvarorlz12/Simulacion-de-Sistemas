...
# Compilación
g++ -O2 lanzamonedas-mod.cpp -o lanzamonedas-mod
...
for i in ${RS[@]}; do
    ...
    ./lanzamonedas-mod $i $PL $PF $P $K $ITER >> $resultados 2>> $errores
done
...
for pf in ${PFS[@]}; do
    ...
    for pl in ${PLS[@]}; do
        ./lanzamonedas-mod 100000 $pl $pf $P $K 1 >> $resultados 2>> $errores
    done
done
...
for k in ${KS[@]}; do
    ./lanzamonedas-mod 100000 $PL $PF $P $k 1 >> $fichero_mod_k 2>> $errores
done
echo -e "\t- Fijado k=$K, variando p"
fichero_mod_p="$carpeta/resultados_k$K-p-var.dat"
for p in ${PS[@]}; do
    ./lanzamonedas-mod 100000 $PL $PF $p $K 1 >> $fichero_mod_p 2>> $errores
done
echo -e "\t- Variando p y k"
fichero_mod="$carpeta/resultados_p-k-var.dat"
for pi in ${PS3D[@]}; do
    for ki in ${KS[@]}; do
        ./lanzamonedas-mod 100000 $PL $PF $pi $ki 1 >> $fichero_mod 2>> $errores
    done
