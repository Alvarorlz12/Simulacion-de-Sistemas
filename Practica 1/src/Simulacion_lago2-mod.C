...
//modelo del lago con peces chicos y grandes, cambiando los par�metros
...
int main(int argc, char * argv[]) {
	...
	if (argc != 5 && argc !=8) {
		...
		cerr << "Uso: " << argv[0] << " tfin x0 y0 f [dias \%p_peq \%p_gra]" << endl;
		exit(0);
	} 
	...
	if (argc == 8) {
		__P_dias = atof(argv[5]);
		if (__P_dias <= 0.0 || __P_dias > _tfin) {
			cerr << "Error: nº de días de ciclo de pesca incorrecto" << endl;
			exit(0);
		}
		__P_peq = atof(argv[6]);
		__P_gra = atof(argv[7]);
